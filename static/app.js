...
function render(){
   ...
   active_section1 = 'recognize-content';

    $('#'+active_section).show();
    $('#'+active_section1).show();
}
function update(){
...
$(document).ready(function(){
    ...
    $('#recognize').submit(function(e){
        ...
        axios.post('/api/recognize', recog_form_data).then(function(response){
            ...
            console.log("We found a user matched with your face image1 is", response.data);

            message = {type: 'success', message: 'We found a face matched with uploaded face and the details is : '+ response.data.user.name};
